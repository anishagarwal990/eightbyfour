    ...
    <Reveal as="section" className="px-7 py-16 md:py-24">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-baseline justify-between gap-x-8 gap-y-3">
          ...
          <Link href="/products" className="text-sm underline underline-offset-4" style={{ color: "var(--burgundy)" }}>
            Browse the full catalogue
          </Link>
        </div>

        <div className="mt-10 grid grid-cols-2 gap-x-4 gap-y-8 md:gap-x-6 lg:grid-cols-4">
          {/* Phase offset per tile so the grid never flips as one block. */}
          {[first, second].filter(Boolean).map((tile, i) => (
            <FeatureTile key={tile.slug} tile={tile} delayMs={i * 900} />
          ))}
          {others.map((tile, i) => (
            <SmallTile key={tile.slug} tile={tile} delayMs={(i + 2) * 900} />
          ))}
        </div>
          ...
          <div className="mt-14 border-t pt-8" style={{ borderColor: "var(--line)" }}>
            <div className="grid grid-cols-1 gap-x-10 gap-y-4 md:grid-cols-[0.55fr_1fr]">
              <div>
                ...
                <p className="mt-2 max-w-xs" style={{ fontSize: "var(--fs-body-sm)", lineHeight: "var(--lh-normal)", color: "var(--line-strong)" }}>
                ...
                {sourced.map((c) => (
                  <li key={c.slug}>
                    <Link
                      ...
                      className="text-[14px] transition-colors hover:text-[var(--burgundy)]"
...
/* Every image sits on the same neutral ground with the same crop rule, which
   is what holds a catalogue of texture photos, product PNGs and packaging
   shots to one art direction. No outline around it: a full-bleed texture
   needs no edge drawn, and on a packshot the ground already supplies one. */
const FRAME_STYLE = {
  background: "var(--card)",
...
function imageClass(treatment: ImageTreatment): string {
  return treatment === "surface" ? "object-cover" : "object-contain p-5 md:p-7";
}

function FeatureTile({ tile, delayMs }: { tile: DiscoveryTile; delayMs: number }) {
  return (
    <Link href={`/products/${tile.slug}`} className="group col-span-2 flex flex-col gap-3">
      <span className="relative block aspect-[4/3] w-full overflow-hidden" style={FRAME_STYLE}>
        {tile.images.length > 0 ? (
          <CyclingImage
            ...
            imageClassName={imageClass(tile.treatment)}
            delayMs={delayMs}
          />
        ) : null}
      </span>
      <span>
        <span className="flex items-baseline gap-3">
          <span className="font-display text-[19px] font-semibold transition-colors group-hover:text-[var(--burgundy)]">
            {tile.name}
          </span>
          <span className="metric text-[15px]" style={{ color: "var(--burgundy)" }}>
            {tile.count.toLocaleString("en-IN")}
          </span>
        </span>
        <span className="mt-1 block max-w-sm" style={{ fontSize: "var(--fs-body-sm)", lineHeight: "var(--lh-normal)", color: "var(--line-strong)" }}>
          {tile.blurb}
        </span>
      </span>
    </Link>
...
function SmallTile({ tile, delayMs }: { tile: DiscoveryTile; delayMs: number }) {
  return (
    <Link href={`/products/${tile.slug}`} className="group flex flex-col gap-2.5">
      ...
      <span className="flex items-baseline justify-between gap-2">
        <span className="text-[14px] font-medium transition-colors group-hover:text-[var(--burgundy)]">{tile.name}</span>
        <span className="metric text-[13px]" style={{ color: "var(--line-strong)" }}>
          {tile.count.toLocaleString("en-IN")}
        </span>
      </span>
    </Link>
